...
def get_list():
    sql = text("""SELECT E.restaurant, R.feedback, R.date, R.type, R.food, R.atmosphere, R.service, U.username, R.sent_at, R.id 
                    ...
                    WHERE R.user_id=U.id AND R.restaurant_id=E.id AND visible=1 
                    ORDER BY R.id DESC""")
...
def sendreview(restaurant, date, type, food, atmosphere, service, feedback):
    ...
    if user_id == 0:
        return False
    sql1 = text("""INSERT INTO restaurants (restaurant) 
                    VALUES (:restaurant)""")
    db.session.execute(sql1, {"restaurant":restaurant})
    restaurant_id = restaurants.get_id(restaurant)
    sql2 = text("""INSERT INTO reviews (restaurant_id, date, type, food, atmosphere, service, feedback, user_id, sent_at, visible) 
                VALUES (:restaurant_id, :date, :type, :food, :atmosphere, :service, :feedback, :user_id, NOW(), 1)""")
